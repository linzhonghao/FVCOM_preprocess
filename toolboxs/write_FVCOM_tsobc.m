function write_FVCOM_tsobc(basename,time,nSiglay,in_temp,in_salt,in_no3,in_phy,in_zoo,Mobj,varargin)
...
if isscalar(in_temp) && isscalar(in_salt)
    ...
    no3 = repmat(in_no3, [nObc, nSiglay, nTimes]);
    phy = repmat(in_phy, [nObc, nSiglay, nTimes]);
%     zoo = repmat(in_zoo, [nObc, nSiglay, nTimes]);
else
...
obc_no3_varid=netcdf.defVar(nc,'Nitrogen','NC_FLOAT',[nobc_dimid,siglay_dimid,time_dimid]);
netcdf.putAtt(nc,obc_no3_varid,'long_name','sea_water_Nitrogen');
netcdf.putAtt(nc,obc_no3_varid,'units','mmole N m-3');
netcdf.putAtt(nc,obc_no3_varid,'grid','obc_grid');

obc_phy_varid=netcdf.defVar(nc,'Phytoplankton','NC_FLOAT',[nobc_dimid,siglay_dimid,time_dimid]);
netcdf.putAtt(nc,obc_phy_varid,'long_name','sea_water_Phytoplankton');
netcdf.putAtt(nc,obc_phy_varid,'units','mmole C m-3');
netcdf.putAtt(nc,obc_phy_varid,'grid','obc_grid');

obc_zoo_varid=netcdf.defVar(nc,'Zooplankton','NC_FLOAT',[nobc_dimid,siglay_dimid,time_dimid]);
netcdf.putAtt(nc,obc_zoo_varid,'long_name','sea_water_Zooplankton');
netcdf.putAtt(nc,obc_zoo_varid,'units','mmole C m-3');
netcdf.putAtt(nc,obc_zoo_varid,'grid','obc_grid');
%%
...
netcdf.putVar(nc,obc_no3_varid,no3);
netcdf.putVar(nc,obc_phy_varid,phy);
% netcdf.putVar(nc,obc_zoo_varid,zoo);
% close file
netcdf.close(nc);
